...
You are running the V2P production hardening pipeline. Follow these steps exactly.

## Step 1: Check Status
Call `v2p_status` to see if a target project is loaded and current state.

If no target is loaded, ask the user for the path to their project.

## Step 2: Scan for Defects
Call `v2p_scan` to discover production readiness defects.
...
## Step 6: Chaos Test
Call `v2p_chaos` to verify fixes hold under adversarial conditions.

Report chaos resilience percentage and any failures.

## Step 7: Final Score
Call `v2p_score` with `detail: true, antifragile: true` to show the final antifragility score.

## Step 8: Generate Report
Call `v2p_report` to produce the stakeholder HTML report.
